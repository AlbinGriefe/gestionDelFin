...
export default function PersonsList({ onEdit, onWorkflow }: PersonsListProps) {
  const { persons, loading, pagination, filters, setFilters, catalogs } =
    usePersons();
  ...
  const handleProfession = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setFilters({
      ...filters,
      page: 1,
      professionId: val ? Number(val) : undefined,
    });
  };
  ...
  const handleHealth = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setFilters({
      ...filters,
      page: 1,
      healthId: val ? Number(val) : undefined,
    });
  };
  ...
  const handleAccepted = (val: string) => {
    setFilters({
      ...filters,
      page: 1,
      accepted: val === "" ? undefined : val === "true",
    });
  };
  ...
  const currentAccepted =
    filters.accepted === undefined ? "" : String(filters.accepted);
  const currentActive =
          ...
          {pagination && pagination.totalPages > 1 && (
            <div className={styles.pagination}>
              <span className={styles.paginationInfo}>
                ...
                {pagination.totalItems} personas
              </span>
