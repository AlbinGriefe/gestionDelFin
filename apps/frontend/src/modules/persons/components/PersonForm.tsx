...
export default function PersonForm({ initialData, onSubmit, onClose }: PersonFormProps) {
    ...
    const handleSubmit = async () => {
        ...
        if (profileDescription.trim() && profileDescription.trim().length < 20) {
            return setError("La descripcion del perfil debe tener al menos 20 caracteres.");
        }
            ...
            <div className={styles.modal} onClick={event => event.stopPropagation()}>
                <div className={styles.header}>
                    <span className={styles.title}>
                        ...
                        {isEditing && (
                            <span className={styles.subtitle}> - {initialData?.fullName}</span>
                        )}
                    </span>
                ...
                <div className={styles.body}>
                    ...
                    <div className={styles.formRow}>
                        <div>
                            <label className={styles.label}>
                                Notas de admision <span className={styles.optional}>(opcional)</span>
                            </label>
                            <input
                                className={styles.input}
                                value={notes}
                    ...
                    {!isEditing && (
                        <div className={styles.aiPlaceholder}>
                            ...
                            <p className={styles.aiDescription}>
                                La persona se registra como pendiente y sin oficio. La evaluacion
                                textual de admision y la recomendacion de oficio se confirman por
                                separado para conservar la decision humana.
                            </p>
                        </div>
                    )}
