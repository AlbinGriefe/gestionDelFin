...
export function PersonsProvider({ children }: { children: React.ReactNode }) {
  ...
  const [pagination, setPagination] = useState<PersonList["pagination"] | null>(
    null,
  );
  const [appliedFilters, setAppliedFilters] = useState<
    PersonList["appliedFilters"] | null
  >(null);
  ...
  const getPersonById = useCallback(
    async (personId: number): Promise<PersonDetail> => {
  ...
  const createPerson = useCallback(
    async (data: PersonWriteInput): Promise<PersonDetail> => {
  ...
  const updatePerson = useCallback(
    async (personId: number, data: PersonWriteInput): Promise<PersonDetail> => {
