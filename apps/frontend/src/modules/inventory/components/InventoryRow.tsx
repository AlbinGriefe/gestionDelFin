...
import type { InventorySummary, InventoryDetail } from "../types/inventory.types";
...
export default function InventoryRow({ item, onAdjust, onThresholds }: InventoryRowProps) {
    ...
    useEffect(() => {
        ...
        getInventoryById(item.storageId)
            .then(setDetail)
            .catch(() => {})
            .finally(() => setLoadingDetail(false));
    }, [isOpen, detail, item.storageId, getInventoryById]);

    return (
        <>
            <tr className={styles.row} onClick={() => setIsOpen(p => !p)}>
                ...
                <td className={styles.td}>
                    <span
                        ...
                        style={item.resource.type.isPriority
                            ? { background: "#FEF3E2", color: "#7A4500" }
                            : { background: "#F4F4F2", color: "#555" }
                        }
                    >
                        {item.resource.type.isPriority ? "⚑ " : ""}{item.resource.type.name}
                    </span>
                </td>
                <td className={styles.td}>
                ...
                <td className={styles.td}>
                    <span className={styles.threshold}>
                        {item.minQuantity}
                        {item.maxQuantity !== null && <span className={styles.unit}> / {item.maxQuantity}</span>}
                        <span className={styles.unit}> {item.resource.unit}</span>
                    </span>
                </td>
                            ...
                            <div>
                                <div className={styles.detailGrid}>
                                    <DetailCard title="Recurso">
                                        ...
                                        <DetailField label="Tipo" value={detail.resource.type.name} />
                                        <DetailField label="Racionable" value={detail.resource.isRationable ? "Sí" : "No"} />
                                        <DetailField label="Prioritario" value={detail.resource.type.isPriority ? "Sí" : "No"} />
                                    </DetailCard>
                                    <DetailCard title="Niveles">
                                        <DetailField label="Cantidad actual" value={`${detail.quantity} ${detail.resource.unit}`} />
                                        <DetailField label="Mínimo" value={`${detail.minQuantity} ${detail.resource.unit}`} />
                                        <DetailField
                                            label="Máximo"
                                            value={detail.maxQuantity !== null
                                                ? `${detail.maxQuantity} ${detail.resource.unit}`
                                                : "Sin límite"
                                            }
                                        />
                                        <DetailField label="Actualizado" value={fmt(detail.lastUpdatedAt)} />
                                    </DetailCard>
                                </div>
                                                ...
                                                <tbody>
                                                    {detail.recentMovements.map(mv => (
                                                        <tr key={mv.id}>
                                                            ...
                                                            <td className={mv.quantity >= 0 ? styles.qtyPositive : styles.qtyNegative}>
                                                                {mv.quantity >= 0 ? "+" : ""}{mv.quantity} {detail.resource.unit}
                                                            </td>
                                                            <td>{mv.reason ?? "—"}</td>
                                            ...
                                            <table className={styles.movTable}>
                                                ...
                                                <tbody>
                                                    {detail.recentRecords.map(rec => (
                                                        <tr key={rec.id}>
                                                            <td>{rec.previousQuantity} {detail.resource.unit}</td>
                                                            <td className={rec.newQuantity >= rec.previousQuantity ? styles.qtyPositive : styles.qtyNegative}>
