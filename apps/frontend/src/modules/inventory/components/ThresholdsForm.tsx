...
import type { InventorySummary, InventoryThresholdsInput } from "../types/inventory.types";
...
export default function ThresholdsForm({ selectedInventory, onClose }: ThresholdsFormProps) {
    const { inventoryThresholds } = useInventory();

    const [minQty, setMinQty] = useState(String(selectedInventory.minQuantity));
    const [maxQty, setMaxQty] = useState(
        selectedInventory.maxQuantity !== null ? String(selectedInventory.maxQuantity) : ""
    );
    ...
    const handleSubmit = async () => {
        ...
        if (isNaN(min) || min < 0) return setError("El mínimo debe ser un número mayor o igual a 0.");

        let max: number | null | undefined = undefined;
        if (maxQty.trim() !== "") {
            const parsed = parseFloat(maxQty);
            if (isNaN(parsed) || parsed < 0) return setError("El máximo debe ser un número mayor o igual a 0.");
            if (parsed < min) return setError("El máximo no puede ser menor que el mínimo.");
            max = parsed;
        } else {
            max = null;
        }
                ...
                <div className={styles.body}>
                    <div className={styles.currentInfo}>
                        ...
                        <div className={styles.infoItem}>
                            ...
                            <span className={styles.infoValue}>
                                {selectedInventory.minQuantity} {selectedInventory.resource.unit}
                            </span>
                        </div>
                        <div className={styles.infoItem}>
                    ...
                    <div className={styles.formRow}>
                        <div>
                            <label className={styles.label}>
                                Mínimo <span className={styles.unit}>({selectedInventory.resource.unit})</span>
                            </label>
                            <input
                                ...
                                placeholder="ej. 50"
                            />
                            <p className={styles.hint}>Se generará alerta cuando la cantidad caiga por debajo de este valor.</p>
                        </div>
                        <div>
                            <label className={styles.label}>
                            ...
                            />
                            <p className={styles.hint}>Dejar vacío para no establecer un límite superior.</p>
                        </div>
                    </div>
