...
async function listInventories(filters: InventoryListFilters) {
    ...
    if (filters?.resourceTypeId) params.append("resourceTypeId", String(filters.resourceTypeId));
    if (filters?.priorityOnly) params.append("priorityOnly", String(filters.priorityOnly));
    if (filters?.belowMinimum) params.append("belowMinimum", String(filters.belowMinimum));
    if (filters?.rationableOnly) params.append("rationableOnly", String(filters.rationableOnly));
...
async function inventoryThresholds(inventoryId: number, data: InventoryThresholdsInput) {
