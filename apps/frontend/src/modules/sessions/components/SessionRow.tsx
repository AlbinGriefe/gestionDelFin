        ...
        <>
            <tr className={styles.row} onClick={() => setIsOpen(prev => !prev)}>
                <td className={styles.td} style={{ fontWeight: 500 }}>
                    ...
                    {session.isCurrent && <span className={styles.currentTag}>actual</span>}
                </td>
                <td className={styles.td}>{session.camp.name}</td>
                <td className={styles.td} style={{ color: "#555" }}>{session.ipAddress}</td>
                <td className={styles.td} style={{ color: "#555" }}>{fmt(session.loginAt)}</td>
                <td className={styles.td} style={{ color: "#555" }}>{fmt(session.expiresAt)}</td>
                ...
                <td className={styles.td}>
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                        ...
                        <button
                            ...
                            title={session.isCurrent ? "No puedes revocar tu sesión actual" : undefined}
                            onClick={e => { e.stopPropagation(); onRevoke(session); }}
                        >
                            Revocar
                        </button>
                    </div>
                    ...
                    <td colSpan={7} className={styles.detailCell}>
                        <div className={styles.detailGrid}>
                            ...
                            <DetailCard title="Sesión">
                                ...
                                <DetailField label="Última actividad" value={fmt(session.lastUpdateAt)} />
                            ...
                            <DetailCard title="Seguridad">
                                ...
                                <DetailField label="Estado" value={isActive ? "Activa" : "Cerrada"} />
                                <DetailField label="Motivo de cierre" value={REASON_LABELS[session.signOutReason] ?? session.signOutReason} />
                            </DetailCard>
                        </div>
                    </td>
                </tr>
