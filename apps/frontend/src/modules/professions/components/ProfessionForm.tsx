...
import type { ProfessionSummary, ProfessionWriteInput } from "../types/professions.types";
...
export default function ProfessionForm({ initialData, onSubmit, onClose }: ProfessionFormProps) {
    ...
    const [collectsResources, setCollectsResources] = useState(initialData?.collectsResources ?? false);
    const [foodPerDay, setFoodPerDay] = useState(initialData?.foodGeneratedPerDay?.toString() ?? "0");
    const [waterPerDay, setWaterPerDay] = useState(initialData?.waterGeneratedPerDay?.toString() ?? "0");
    ...
    const handleSubmit = async () => {
        ...
        if (!name.trim()) return setError("El nombre de la profesión es obligatorio.");
        ...
        if (foodPerDay && (isNaN(Number(foodPerDay)) || Number(foodPerDay) < 0))
            return setError("La comida por día debe ser un número mayor o igual a 0.");
            ...
            <div className={styles.modal} onClick={e => e.stopPropagation()}>
                ...
                <div className={styles.header}>
                    <span className={styles.title}>
                        ...
                        {isEditing && <span className={styles.subtitle}>· {initialData.name}</span>}
                    </span>
                    ...
                    <div className={styles.formRow}>
                        <div>
                            <label className={styles.label}>
                                Campamento <span className={styles.optional}>(vacío = global)</span>
                            </label>
                            <input
                                className={styles.input}
                                value={campId}
                ...
                <div className={styles.footer}>
                    ...
                    <button className={styles.btnPrimary} onClick={handleSubmit} disabled={loading}>
                        {loading ? "Guardando..." : isEditing ? "Guardar cambios" : "Crear profesión"}
                    </button>
                </div>
