...
export function ProfessionsProvider({ children }: { children: React.ReactNode }) {
    const [professions, setProfessions] = useState<ProfessionSummary[]>([]);
    const [pagination, setPagination] = useState<ProfessionList["pagination"] | null>(null);
    const [appliedFilters, setAppliedFilters] = useState<ProfessionList["appliedFilters"] | null>(null);
    ...
    const getProfessionById = async (professionId: number): Promise<ProfessionSummary> => {
        return professionsApi.getProfessionById(professionId);
    };

    const createProfession = async (data: ProfessionWriteInput): Promise<ProfessionSummary> => {
        const created = await professionsApi.createProfession(data);
        await loadProfessions();
        return created;
    };

    const updateProfession = async (professionId: number, data: ProfessionWriteInput): Promise<ProfessionSummary> => {
        const updated = await professionsApi.updateProfession(professionId, data);
        await loadProfessions();
        return updated;
    };

    const getProfessionCoverage = async (campId: number): Promise<ProfessionCoverageResult> => {
