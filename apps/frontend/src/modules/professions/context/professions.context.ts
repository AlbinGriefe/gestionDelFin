...
export interface ProfessionsContextType {
    ...
    updateProfession: (professionId: number, data: ProfessionWriteInput) => Promise<ProfessionSummary>;
    getProfessionCoverage: (campId: number) => Promise<ProfessionCoverageResult>;
    createTemporaryReassignment: (data: TemporaryReassignmentInput) => Promise<ReassignmentResult>;
    revertTemporaryReassignment: (data: RevertReassignmentInput) => Promise<ReassignmentResult>;
}
