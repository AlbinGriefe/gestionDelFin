...
export default function CampForm({ initialData, onSubmit, onClose }: CampFormProps) {
    ...
    const [status, setStatus] = useState<CampStatus>(initialData?.status ?? "active");

    const handleSubmit = async () => {
        setError("");

        if (!name.trim()) return setError("El nombre del campamento es obligatorio.");
        if (!location.trim()) return setError("La ubicación es obligatoria.");
        if (!maxCapacity || Number(maxCapacity) <= 0) return setError("La capacidad máxima debe ser mayor a 0.");
        if (latitude && isNaN(Number(latitude))) return setError("Latitud inválida.");
        if (longitude && isNaN(Number(longitude))) return setError("Longitud inválida.");
            ...
            <div className={styles.modal} onClick={e => e.stopPropagation()}>
                ...
                <div className={styles.footer}>
                    ...
                    <button className={styles.btnPrimary} onClick={handleSubmit} disabled={loading}>
                        {loading ? "Guardando..." : isEditing ? "Guardar cambios" : "Crear campamento"}
                    </button>
                </div>
