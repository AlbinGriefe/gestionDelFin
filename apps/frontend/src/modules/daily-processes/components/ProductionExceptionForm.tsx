...
export default function ProductionExceptionForm({
  onClose,
}: ProductionExceptionFormProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      professionsApi.listProfessions({
        page: 1,
        pageSize: 100,
        active: true,
        campId: user.campId,
  ...
  const selectedPerson =
    persons.find((p) => p.id === Number(selectedPersonId)) ?? null;
  ...
  const handleSubmit = async () => {
    ...
    if (isNaN(realFood) || realFood < 0)
      return setError("El valor de alimento debe ser ≥ 0.");
    if (isNaN(realWater) || realWater < 0)
      return setError("El valor de agua debe ser ≥ 0.");
    ...
    try {
      ...
      if (tasks.length === 0) {
        toast.warning(
          "No se encontraron los recursos de comida/agua en el inventario.",
        );
        onClose();
        return;
      }
      ...
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.body}>
          <p className={styles.description}>
            ...
            esperada. Se aplicará un ajuste negativo al inventario por la
            diferencia.
          </p>
            ...
            <>
              ...
              {profession && (expectedFood > 0 || expectedWater > 0) && (
                <>
                  ...
                  {(parseFloat(actualFood) < expectedFood ||
                    parseFloat(actualWater) < expectedWater) && (
                        ...
                        {expectedFood > 0 &&
                          parseFloat(actualFood) < expectedFood && (
                            <span className={styles.deficitItem}>
                              −
                              {(
                                expectedFood - parseFloat(actualFood || "0")
                              ).toFixed(2)}{" "}
                              alimento
                            </span>
                          )}
                        {expectedWater > 0 &&
                          parseFloat(actualWater) < expectedWater && (
                            <span className={styles.deficitItem}>
                              −
                              {(
                                expectedWater - parseFloat(actualWater || "0")
                              ).toFixed(2)}{" "}
                              agua
                            </span>
                          )}
