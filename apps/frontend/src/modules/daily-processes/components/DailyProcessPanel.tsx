...
const AUTHORIZED_ROLES = ["gestion recursos"];
...
function canRunProcess(roleName: string) {
  ...
  return AUTHORIZED_ROLES.some((r) =>
    norm.includes(normalizeRole(r).split(" ")[0]),
  );
}
...
export default function DailyProcessPanel() {
  ...
  const handleRun = async () => {
    ...
    setRunning(true);
    try {
      const data = await dailyProcessesApi.runDailyProcesses({
        campId: user.campId,
      });
      setResult(data);
      setStatus((prev) =>
        prev ? { ...prev, ranToday: true, lastRunAt: data.runAt } : prev,
      );
      setResultsOpen(true);
      if (data.alreadyRunToday) {
        toast.info(
          "El proceso ya había sido ejecutado hoy. Mostrando resultados guardados.",
        );
      } else {
        ...
        >
          <div className={styles.panelTitle}>
            ...
            {!loadingStatus && (
              <span
                className={
                  status?.ranToday ? styles.statusDone : styles.statusPending
                }
              >
              ...
              <>
                <div className={styles.statusRow}>
                  {status?.ranToday ? (
                    <p className={styles.statusText}>
                      Ejecutado el{" "}
                      {status.lastRunAt ? fmtFull(status.lastRunAt) : "hoy"}.
                      Producción y raciones ya fueron procesadas.
                    </p>
                  ) : (
                    <p className={styles.statusText}>
                      El proceso del día aún no ha sido ejecutado para{" "}
                      <strong>{user?.campName}</strong>. Al ejecutarlo se
                      calculará la producción de todos los trabajadores y se
                      distribuirán las raciones.
                    </p>
                  )}
                </div>
                ...
                <div className={styles.actions}>
                  {authorized && (
                    <button
                      className={
                        status?.ranToday ? styles.btnSecondary : styles.btnRun
                      }
                      onClick={handleRun}
                      disabled={running}
                    >
