...
async function getAssignments(campId: number, date: string) {
  ...
  return httpClient<DailyAssignmentsResult>(
    `/daily-processes/assignments?${params.toString()}`,
  );
}

async function updateAssignments(data: DailyAssignmentsInput) {
  return httpClient<DailyAssignmentsResult>("/daily-processes/assignments", {
    method: "PUT",
    body: JSON.stringify(data),
  });
}
