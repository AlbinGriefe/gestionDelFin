...
export function ExpeditionsProvider({ children }: { children: React.ReactNode }) {
    const [expeditions, setExpeditions] = useState<ExpeditionSummary[]>([]);
    const [pagination, setPagination] = useState<ExpeditionsList["pagination"] | null>(null);
    const [appliedExpeditionsFilters, setAppliedExpeditionsFilters] = useState<ExpeditionsList["appliedFilters"] | null>(null);
    const [catalogs, setCatalogs] = useState<ExpeditionCatalogs | null>(null);

    const [loading, setLoading] = useState(false);

    const [expeditionsFilters, setExpeditionsFilters] = useState<ExpeditionListFilters>({
        page: 1,
        pageSize: 20,
    });

    const [catalogsFilters, setCatalogsFilters] = useState<ExpeditionCatalogFilters>({});
    ...
    const getExpeditionById = async (expeditionId: number): Promise<ExpeditionDetail> => {
        return expeditionApi.getExpeditionById(expeditionId);
    };

    const createExpedition = async (data: ExpeditionCreateInput): Promise<ExpeditionDetail> => {
    ...
    const updateExpeditionState = async (
        ...
        const updated = await expeditionApi.updateExpeditionState(expeditionId, data);
        await loadExpeditions();
        return updated;
    };
