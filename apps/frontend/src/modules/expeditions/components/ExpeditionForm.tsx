...
import type { ExpeditionCreateInput, ExpeditionMemberInput } from "../types/expeditions.types";
...
export default function ExpeditionForm({ onSubmit, onClose }: ExpeditionFormProps) {
    ...
    const handleAddMember = () => {
        ...
        setMembers(prev => [
            ...prev,
            { id_person: person.id, fullName: person.fullName, rationsAssigned: "0", roleInExpedition: "" },
        ]);
        setPersonToAdd("");
    };
    ...
    const updateMember = (personId: number, patch: Partial<DraftMember>) => {
        setMembers(prev => prev.map(m => (m.id_person === personId ? { ...m, ...patch } : m)));
    };
    ...
    const handleSubmit = async () => {
        ...
        if (!days || days < 1) return setError("Los días estimados deben ser al menos 1.");
        if (members.length === 0) return setError("Agrega al menos un integrante a la expedición.");
                                    ...
                                    <div className={styles.memberFields}>
                                        <div>
                                            ...
                                            <input
                                                ...
                                                onChange={e => updateMember(m.id_person, { rationsAssigned: e.target.value })}
                                            />
                                        </div>
                                        <div>
                                            ...
                                            <input
                                                ...
                                                onChange={e => updateMember(m.id_person, { roleInExpedition: e.target.value })}
