...
const NEXT_STATES_BY_CURRENT: Record<string, { value: NextState; label: string }[]> = {
    planned: [
...
export default function ExpeditionStateModal({ expedition, onConfirm, onClose }: ExpeditionStateModalProps) {
    ...
    const [nextState, setNextState] = useState<NextState>(options[0]?.value ?? "cancelled");
    ...
    useEffect(() => {
        ...
        getExpeditionById(expedition.id)
            ...
            .catch(() => {
                if (active) toast.error("No se pudo cargar el detalle de la expedición");
            })
            .finally(() => {
    ...
    const handleSubmit = async () => {
        ...
        if (nextState === "returned" || nextState === "failed") {
            ...
            if (resourcesReturned) payload.exe_resources_returned = Number(resourcesReturned);
        }

        if (isReturn) {
            if (arrivingDate) payload.exs_arriving_date = new Date(arrivingDate);

            const members: ExpeditionReturnMemberInput[] = Object.entries(memberFindings)
            ...
            <div className={styles.modal} onClick={e => e.stopPropagation()}>
                ...
                <div className={styles.body}>
                        ...
                        <div className={styles.formRow}>
                            <div>
                                <label className={styles.label}>
                                    Recursos usados <span className={styles.optional}>(opcional)</span>
                                </label>
                                <input
                                    className={styles.input}
                                    type="number"
                            ...
                            <div>
                                <label className={styles.label}>
                                    Recursos traídos <span className={styles.optional}>(opcional)</span>
                                </label>
                                <input
                                    className={styles.input}
                                    type="number"
                    ...
                    {isReturn && (
                        <div className={styles.formGroup}>
                            <label className={styles.label}>
                                Fecha de regreso <span className={styles.optional}>(opcional)</span>
                            </label>
                            <input
                                className={styles.input}
                                type="datetime-local"
                    ...
                    {isReturn && (
                        <>
                            <p className={styles.sectionTitle}>Recursos encontrados por integrante</p>
                            {loadingDetail ? (
                                    ...
                                    {detail?.members.map(m => (
                                        <div key={m.personId} className={styles.memberCard}>
                                            ...
                                            <input
                                                ...
                                                onChange={e =>
                                                    setMemberFindings(prev => ({ ...prev, [m.personId]: e.target.value }))
                                                }
                                            />
                                        </div>
                                    ))}
