...
export default function TransfersList({ onManage }: TransfersListProps) {
    const { transfers, loading, pagination, transfersFilters, setTransfersFilters } =
        useTransfers();
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        setTransfersFilters({ ...transfersFilters, page: 1, search: search.trim() || undefined });
    };

    const handleState = (state: StateFilter) => {
        setTransfersFilters({ ...transfersFilters, page: 1, state: state || undefined });
    };

    const handleType = (type: TypeFilter) => {
        setTransfersFilters({ ...transfersFilters, page: 1, type: type || undefined });
    };
                    ...
                    {pagination && pagination.totalPages > 1 && (
                        <div className={styles.pagination}>
                            <span className={styles.paginationInfo}>
                                Página {pagination.page} de {pagination.totalPages} · {pagination.totalItems} solicitudes
                            </span>
