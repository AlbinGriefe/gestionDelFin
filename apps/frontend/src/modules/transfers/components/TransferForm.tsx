...
type DraftPerson = { id_person: number; fullName: string; assignedRations: string };
type DraftResource = { id_resource: number; name: string; unit: string; available: number; quantity: string };
...
export default function TransferForm({ onSubmit, onClose }: TransferFormProps) {
    ...
    const handleAddPerson = () => {
        ...
        if (!person) return;
        setPersons(prev => [...prev, { id_person: person.id, fullName: person.fullName, assignedRations: "0" }]);
        setPersonToAdd("");
    };
    ...
    const handleAddResource = () => {
        ...
        setResources(prev => [
            ...prev,
            { id_resource: resource.id, name: resource.name, unit: resource.unit, available: resource.availableQuantity, quantity: "" },
        ]);
        setResourceToAdd("");
    };
    ...
    const handleSubmit = async () => {
        ...
        for (const r of resources) {
            ...
            if (!qty || qty <= 0) return setError(`Indica una cantidad válida para "${r.name}".`);
            if (qty > r.available) return setError(`No hay suficiente "${r.name}" (disponible: ${r.available}).`);
        }
                    ...
                    {showPersons && (
                        <>
                            ...
                            <div className={styles.addRow}>
                                ...
                                <button type="button" className={styles.addBtn} onClick={handleAddPerson} disabled={!personToAdd}>
                                    Agregar
                                </button>
                            </div>
                            {persons.length === 0 ? (
                                        ...
                                        <div key={p.id_person} className={styles.itemCard}>
                                            ...
                                            <div className={styles.itemField}>
                                                ...
                                                <input
                                                    ...
                                                    onChange={e =>
                                                        setPersons(prev => prev.map(x =>
                                                            x.id_person === p.id_person ? { ...x, assignedRations: e.target.value } : x))
                                                    }
                                                />
                                            </div>
                                            <button
                                                ...
                                                onClick={() => setPersons(prev => prev.filter(x => x.id_person !== p.id_person))}
                                            >
                                                ✕
                                            </button>
                                        </div>
                    ...
                    {showResources && (
                        <>
                            ...
                            <div className={styles.addRow}>
                                ...
                                <button type="button" className={styles.addBtn} onClick={handleAddResource} disabled={!resourceToAdd}>
                                    Agregar
                                </button>
                            </div>
                            {resources.length === 0 ? (
                                    ...
                                    {resources.map(r => (
                                        <div key={r.id_resource} className={styles.itemCard}>
                                            <span className={styles.itemName}>
                                                {r.name} <span className={styles.itemHint}>· disp. {r.available} {r.unit}</span>
                                            </span>
                                            <div className={styles.itemField}>
                                                ...
                                                <input
                                                    ...
                                                    onChange={e =>
                                                        setResources(prev => prev.map(x =>
                                                            x.id_resource === r.id_resource ? { ...x, quantity: e.target.value } : x))
                                                    }
                                                />
                                            </div>
                                            <button
                                                type="button"
                                                className={styles.removeBtn}
                                                onClick={() => setResources(prev => prev.filter(x => x.id_resource !== r.id_resource))}
                                            >
                                                ✕
                                            </button>
                                        </div>
