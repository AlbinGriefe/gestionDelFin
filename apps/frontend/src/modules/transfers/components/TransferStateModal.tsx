...
const STATE_META: Record<
  string,
  { label: string; from: "origen" | "destino" }
> = {
...
export default function TransferStateModal({
  ...
  onConfirm,
  onClose,
}: TransferStateModalProps) {
  const options = ALLOWED_TRANSITIONS[transfer.state] ?? [];

  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [nextState, setNextState] = useState<TransferState>(
    options[0] ?? "cancelled",
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(
        "No se pudo actualizar. Verifica que tu campamento tenga la autoridad para esta acción.",
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className={styles.body}>
          ...
          {meta && (
            <p className={styles.authorityHint}>
              Esta acción debe realizarse desde el{" "}
              <strong>campamento {meta.from}</strong>.
            </p>
          )}
