...
async function loadCatalogs(filters: TransferCatalogFilters) {
    ...
    if (filters?.originCampId) params.append("originCampId", String(filters.originCampId));

    return httpClient<TransferCatalogs>(`/transfers/catalogs?${params.toString()}`);
}
...
async function updateTransferState(transferId: number, data: TransferStateUpdateInput) {
